...
public:
    ...
    const std::set<std::pair<int, int>> getCellIndices() {
        return cellIndices;
    };
    std::set<int> getValues(std::pair<int, int> index) {
        return cells[index];
    };
    ...
    void inferAC3();
    bool isCandidateValid(int candidate, std::set<std::pair<int, int>> neighbors);
